...
class PostModel extends Model
{
    ...
    public function listItems($params, $option){
        ...
        if($option['task'] == 'admin-list-items'){
            $query = $this->select('id', 'name', 'content', 'thumb', 'subject_id', 'status', 'created_at', 'updated_at');
        ...
        if($option['task'] == 'front-end-post-list-items'){
            $result = $this->select('id', 'name', 'thumb', 'viewed', 'subject_id')
    ...
    public function getItem($params = null, $option = null){
        ...
        if($option['task'] == 'get-item'){
            $result = self::select('id', 'name', 'content', 'thumb', 'subject_id', 'status')->where('id', $params['id'])->first();
        }
        if($option['task'] == 'get-thumb'){
            $result = self::select('id', 'thumb')->where('id', $params['id'])->first();
        }
        if($option['task'] == 'front-end-post-detail'){
            $result = self::select('id', 'name', 'content', 'thumb', 'subject_id')->where('id', $params['post_id'])->first();
        }
        return $result;
    }
